...
function Bam (bamUri, options) {
    ...
    this.bamUri = bamUri;
      ...
      if (typeof(this.bamUri) == "object") {
          ...
          // *** add if statement if here ***
          this.baiBlob = new BlobFetchable(this.options.bai);
          this.promises = [];
          this.bam = undefined;
          var me = this;
...
_.extend(Bam.prototype,{
   ...
   fetch: function( name, start, end, callback, options ) {
      ...
      if(this.bam == undefined) // **** TEST FOR BAD BAM ***
         this.promise(
             function() {me.fetch(name, start, end, callback, options);});
      else
   ...
   _getBamRegionsUrl: function(regions) {
      if ( this.sourceType == "url") {
         ...
         // creates a url for a new bam that is sliced from an old bam
          //
         // open connection to iobio webservice that will request this
         // data, since connections can only be opened from browser
          var me = this;
          var connectionID = this._makeid();
          var client = BinaryClient(this.iobio.samtools + '?id=',
                                    {'connectionID' : connectionID});

          client.on('open',
                    function(stream){
                        var stream = client.createStream(
                            {event:'setID', 'connectionID':connectionID});
                        stream.end();
                    });

          var url = this.iobio.samtools +
                    "?protocol=websocket&encoding=binary&cmd=view -S -b " +
                    encodeURIComponent("http://client?&id="+connectionID);
   ...
   getHeader: function(callback) {
      ...
      if (me.header)
         ...
         var url =
              encodeURI( me.iobio.samtools + '?cmd=view -H ' + this.bamUri);
         client.on('open', function(stream){
            var stream =
                 client.createStream({event:'run', params: {'url':url}});
   ...
   samplingRegions: function (SQs, options) {
       options = $.extend({
       ...
       for (var j=0; j < SQs.length; j++) {
           ...
           if ( length < options.binSize * options.binNumber) {
               ...
               //var regions = []; // WTF is the deal here!??!
               for (var i=0; i < options.binNumber; i++) {
                   ...
                   if (s) {
                     ...
                     console.log("ERROR: region start s undefined",
                                 i, s, sqStart, SQs[j].name);
                   };
               }
               // sort by start value
               ...
               if (options.bed != undefined)
                   bedRegions =
                   me._mapToBedCoordinates(SQs[0].name, regions, options.bed)
           }
       }

       return {regions: regions,
               bedRegions: bedRegions,
               regStr: JSON.stringify((bedRegions || regions).map(
                   function(d){return {start:d.start,end:d.end,chr:d.name};}))
              };
   },

   bamStatsAliveSamplingURL: function (SQs, options) {
       var me = this;
       var regionInfo = me.samplingRegions(SQs, options);
       var regions = regionInfo.regions;
       var regStr = regionInfo.regStr;

       // var url = encodeURI( me.iobio.bamstatsAlive + '?cmd=-u
       // 30000 -f 2000 -r \'' + regStr + '\' ' +
   ...
   sampleStats: function(callback, options) {
      ...
      if ( options.sequenceNames != undefined && options.end != undefined) {
         var sqs = options.sequenceNames.map(
             function(d) { return {name:d, end:options.end}});
          goSampling(sqs);
